...
echo '<pre style="font-size: 24px; font-style: italic; color: rgb(64,64,64);">' >> t_treeDiagram.html;
echo '<span style="font-size: 48px; color: #DEFF1C;">HW TREE DIAGRAM</span>' >> t_treeDiagram.html;
find hw/ | sed 's/\(^.*\/\)\(.*$\)/\1<a style="color: #DEFF1C; font-style: normal; font-weight: bold;" href="\1\2">\2<\/a>/g' | sed '/\.js/!i <a>' >> t_treeDiagram.html;
echo "</pre>" >> t_treeDiagram.html;
echo "<script>" >> t_treeDiagram.html;
echo "document.body.style.backgroundColor = 'black';" >> t_treeDiagram.html;
echo "</script>" >> t_treeDiagram.html;
echo "</body>" >> t_treeDiagram.html;
echo "</html>" >> t_treeDiagram.html;
...
head -3 index.html >> hw/z_hwObject/c_startup.js;
...
head -n -3 index.html | tail -n +4 >> hw/z_hwObject/c_startup.js;
