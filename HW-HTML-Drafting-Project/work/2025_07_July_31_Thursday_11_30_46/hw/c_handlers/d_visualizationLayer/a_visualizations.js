...
if (zStack[j].id == mousemoveTarget||(mousedown.hold==true&&zStack[j].id==coinFocus)) {
Y.lineWidth = edgeQ;
} else {
Y.lineWidth = 12;
}
...
w = parseInt(zStack[j].id.style.width) * parseInt(zStack[j].id.dataset.scale) - deduction;
h = parseInt(zStack[j].id.style.height) * parseInt(zStack[j].id.dataset.scale) - deduction;
