...
# Force universal byte-order sorting for all commands in this script
export LC_ALL=C
...
cat storageKeys.txt >>workingVersion.html;
...
# FIX: Added 'sort' and quoted the date string to prevent the 'invalid option' error
find h_hdub -type f | sort | sed '/js\.swp/d' | sed -n '/\.js/p' >> chalk_tmp_filenames; 
find h_hdub -type f | sort | sed '/js\.swp/d' | sed -n '/\.js/p' | sed 's/^/echo -n "?v=$(date -r /' | sed 's/$/ "+%Y%m%d%H%M%S")" >> chalk_tmp_versions;/' > chalk_tmp_versionControl; 
...
find h_hdub -type f -name '*.js' | sort | sed "/shadowTree/d" | sed "s/^/.\//g" | tr '\n' ' ' >> bundleMaker;
...
if [ -s asset_list.txt ]; then
    ...
    find "$IMG_DIR" -name "*.png" | sort | xargs -P 8 -I {} oxipng -o 4 --strip safe {} 2>/dev/null
    find "$IMG_DIR" -name "*.gif" | sort | xargs -P 8 -I {} gifsicle -O3 --lossy=80 {} -o {} 2>/dev/null
...
cat storageKeys.txt >> index.html;
