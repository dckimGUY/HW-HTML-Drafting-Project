function rotateOver() {
    if (coinFocus != null) {
        ...
        let newRotateX = rotateX + rotationStep;
        if (newRotateX >= 360) newRotateX -= 360;
        else if (newRotateX < 0) newRotateX += 360;
        coinFocus.lastElementChild.dataset.rotateX = newRotateX;
        const transformStr = "perspective(" + perspective + "px) rotateX(" + newRotateX + "deg) rotateY(" + rotateY + "deg) rotateZ(" + rotateZ + "deg) translateZ(" + translateZ + "px)";
